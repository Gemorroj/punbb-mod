...
if (!$pun_user['is_guest'] && 1 == $pun_user['g_pm'] && $pun_config['o_pms_enabled'] && 1 == $pun_user['messages_enable']) {
    ...
    $result_messages = $db->query('SELECT COUNT(1) FROM '.$db->prefix.'messages WHERE showed=0 AND owner='.$pun_user['id']);
    if (!$result_messages) {
        \error('Unable to check for new messages', __FILE__, __LINE__, $db->error());
    }
    ...
    if ($new_msg[0] > 0) {
        ...
        if (1 == $pun_user['popup_enable']) {
            $result = $db->query('SELECT id FROM '.$db->prefix.'messages WHERE popup=0 AND owner='.$pun_user['id'].' ORDER BY id DESC');
            if (!$result) {
                \error('Unable update popup status', __FILE__, __LINE__, $db->error());
            }
    ...
    if (!$result) {
        \error('Unable to test if the message-box is full', __FILE__, __LINE__, $db->error());
    }
