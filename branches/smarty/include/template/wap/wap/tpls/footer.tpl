...
{* Index and Userlist should always be displayed *}
    ...
    {if $pun_config.o_rules == 1}
            {$lang_common.$Link_separator}<a href="misc.php?action=rules">{$lang_common.Rules}</a>
    {/if}
    
    {if $pun_user.g_search == 1 or $pun_user.g_id > $smarty.const.PUN_MOD}
            {$lang_common.$Link_separator}<a href="search.php">{$lang_common.Search}</a>
    {/if}

    {if ! $pun_user.is_guest}
            {$lang_common.$Link_separator}<a href="uploads.php">{$lang_common.Uploader}</a>
            {$lang_common.$Link_separator}<a href="filemap.php">{$lang_common.Attachments}</a>
    {/if}
</div>
{/if}
...
{if isset($is_admmod)}
    ...
    <div class="con">
            <span class="sub">
                <a href="moderate.php?fid={$forum_id}&amp;tid={$id}&amp;p={$p}">{$lang_common.$Delete_posts}</a>{$lang_common.$Link_separator}
                <a href="moderate.php?fid={$forum_id}&amp;move_topics={$id}">{$lang_common.$Move_topic}</a>
                {if $cur_topic.closed == 1}
                        {$lang_common.$Link_separator}<a
                            href="moderate.php?fid={$forum_id}&amp;open={$id}">{$lang_common.$Open_topic}</a>
                    {else}
                        {$lang_common.$Link_separator}<a
                            href="moderate.php?fid={$forum_id}&amp;close={$id}">{$lang_common.$Close_topic}</a>
                {/if}

                {if $cur_topic.sticky == 1}
                        {$lang_common.$Link_separator}<a
                            href="moderate.php?fid={$forum_id}&amp;unstick={$id}">{$lang_common.$Unstick_topic}</a>
                    {else}
                        {$lang_common.$Link_separator}<a
                            href="moderate.php?fid={$forum_id}&amp;stick={$id}">{$lang_common.$Stick_topic}</a>
                {/if}
            </span>
    </div>
    {/if}
