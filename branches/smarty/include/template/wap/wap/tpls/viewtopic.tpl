...
<div class="{if $j = ! $j}msg{else}msg2{/if}">
    ...
    {* Номер сообщения и имя пользователя *}
        <a href="viewtopic.php?pid={$cur_post.id}#p{$cur_post.id}">#{($start_from + $post_count)}
            .</a>&#160;<strong>{if $cur_post.poster_id > 1}<a
            href="profile.php?id={$cur_post.poster_id}">{$cur_post.username|escape}</a>{else}{$cur_post.username|escape}{/if}
    </strong>
    ...
    {* Когда было размещено сообщение *}
        ...
        {* Административные ссылки для управления сообщением *}
            <a href="delete.php?id={$cur_post.id}">{$lang_topic.Delete_m}</a>{$lang_topic.$Link_separator_m}<a
                href="edit.php?id={$cur_post.id}">{$lang_topic.Edit_m}</a>{$lang_topic.$Link_separator_m}<a
                href="post.php?tid={$id}&amp;qid={$cur_post.id}">{$lang_topic.Quote_m}</a>{$lang_topic.$Link_separator_m}
