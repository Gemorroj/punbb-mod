...
{if $pun_user.is_guest}
    ...
    <a href="profile.php?id={$pun_user.id}">{$lang_common.Profile} (<span
            style="font-weight: bold">{$pun_user.username|escape}</span>)</a>
...
{* Тут какая-то фигня. Был знак ">" и не работало. Может я когда переводил напутал сам, или так было. *}
    ...
    {/if}
        ...
        {$Link_separator_m}<a
            href="login.php?action=out&amp;id={$pun_user.id}&amp;csrf_token={$logout}">{$lang_common.Logout}</a>
{/if}
</div>
...
<div class="{if $j = ! $j}in{else}in2{/if}">
    {if $cur_forum.redirect_url}
        ...
        {else}
        <a href="viewforum.php?id={$cur_forum.fid}">{$cur_forum.forum_name|escape}</a> ({$cur_forum.num_topics}
        /{$cur_forum.num_posts})
    {/if}
    ...
    {if $cur_forum.last_post}
        ...
        <span class="sub">
        &#187; <a
                href="viewtopic.php?pid={$cur_forum.last_post_id}#p{$cur_forum.last_post_id}">{$cur_forum.subject|escape}</a>&#160;({$cur_forum.last_post|date_format:$date_format}
