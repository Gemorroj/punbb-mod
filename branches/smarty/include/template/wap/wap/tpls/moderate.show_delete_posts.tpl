...
<div class="inbox">
    <a href="index.php">{$lang_common.Index}</a> &#187; <a href="viewforum.php?id={$fid}">{$cur_topic.forum_name|escape}</a> &#187; {$cur_topic.subject|escape}</div>
...
{foreach from=$posts item=cur_post}
    ...
    <div class="zag_in">
    <a href="viewtopic.php?pid={$cur_post.id}#p{$cur_post.id}">#{($start_from + $post_count)}</a> <strong>{if $cur_post.poster_id > 1}<a href="profile.php?id={$cur_post.poster_id}">{$cur_post.poster|escape}</a>{else}{$cur_post.poster|escape}{/if}</strong> ({$user_title})<br/>
