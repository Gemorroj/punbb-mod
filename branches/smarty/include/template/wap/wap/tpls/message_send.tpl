...
{$lang_pms.$Send_to}<br />
<input type="text" name="req_username" maxlength="25" value="{$username|escape}" tabindex="{assign var='cur_index' value='`$cur_index` + 1'}" /><br />
{$lang_common.Subject}<br />
<input class="longinput" type="text" name="req_subject" value="{$subject|escape}" maxlength="70" tabindex="{assign var='cur_index' value='`$cur_index` + 1'}" /><br />
{$lang_common.Message}<br />
<textarea name="req_message" rows="4" cols="24" tabindex="{assign var='cur_index' value='`$cur_index` + 1'}">{$quote|escape}</textarea><br />
...
{/if}
<input type="checkbox" name="savemessage" value="1" checked="checked" tabindex="{assign var='cur_index' value='`$cur_index` + 1'}" />{$lang_pms.$Save_message}
</div>
<div class="go_to">
<input type="submit" name="submit" value="{$lang_pms.Send}" tabindex="{assign var='cur_index' value='`$cur_index` + 1'}" accesskey="s" />
</div>
</form>

<div class="navlinks">
<a href="message_list.php?box=0">{$lang_pms.Inbox}</a> | <a href="message_list.php?box=1">{$lang_pms.Outbox}</a> | <a href="message_list.php?box=2">{$lang_pms.Options}</a> | <a href="message_send.php">{$lang_pms.$New_message}</a>
</div>
