...
{foreach from=$attachments item=row}
    ...
    {if $row.cat_id != $cur_category || $row.forum_id != $cur_forum}
        <div class="cat">{$categories[$row.cat_id]|escape} &#187; <a href="viewforum.php?id={$row.forum_id}">{$forums[$row.forum_id].forum_name|escape}</a></div>
    ...
    {round($row.size/1024,1)}kb, {if preg_match('|^image/(.*)$|i', $row.mime, $regs)}{$regs[1]} {$row.image_dim}, {/if}downloads: {$row.downloads}
