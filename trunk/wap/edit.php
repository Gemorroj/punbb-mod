...
if ($errors) {
    echo '<div class="red">'.$lang_post['Post errors'].'</div><div class="msg">'.$lang_post['Post errors info'];
...
echo '<div class="con">'.$lang_post['Edit post'].'</div>
<form method="post" action="edit.php?id='.$id.'&amp;action=edit" enctype="multipart/form-data">
<div class="input">
<input type="hidden" name="form_sent" value="1" />';
if ($can_edit_subject) {
    echo $lang_common['Subject'].'<br /><input type="text" name="req_subject" tabindex="'.($cur_index++).'" value="'.pun_htmlspecialchars(isset($_POST['req_subject']) ? $_POST['req_subject'] : $cur_post['subject']).'" /><br /></label>';
}
require PUN_ROOT.'include/attach/fetch.php';

//Message
echo $lang_common['Message'].':<br/><textarea name="req_message" rows="4" cols="24" tabindex="'. $cur_index++ .'">'.pun_htmlspecialchars(isset($_POST['req_message']) ? $message : $cur_post['message']). '</textarea><br />';
//Smilies/BBCode
