...
<div class="box">
<div class="inbox">
<p>Этот плагин используется для очистки от лишних сообщений спаммеров и редактирует базу данных приводя в соответствие не совпадающие вещи.</p>
</div>
</div>
</div>
<div class="block">
...
<div class="inbox">
<p>Позволяет навести порядок после атаки спаммеров. Работает так: вы вводите один или несколько IP адресов (разделены пробелами) и все пользователи и сообщения с этими IP удаляются вместе с другими операциями очистки.</p>
...
<form method="post" action="<?php echo $_SERVER['REQUEST_URI']; ?>">
<div class="inbox">
<p>Исправляет несоответствующие действительности показатели количества сообщений/тем каждого форума (обычно если вы редактировали базу данных в ручную) пересчитать верно.</p>
</div>
<p class="submitend">
...
<form method="post" action="<?php echo $_SERVER['REQUEST_URI']; ?>">
<div class="inbox">
<p>Исправляет сбившееся количество ответов каждой темы (обычно если вы редактировали базу данных в ручную) и пересчитывает верно</p>
</div>
<p class="submitend">
...
<form method="post" action="<?php echo $_SERVER['REQUEST_URI']; ?>">
<div class="inbox">
<p>Исправляет сбившееся количество сообщений каждого пользователя (обычно если вы удаляли какие то сообщения в базе данных) и пересчитывает верно.</p>
</div>
<p class="submitend">
...
<form method="post" action="<?php echo $_SERVER['REQUEST_URI']; ?>">
<div class="inbox">
<p>Исправляет список последних постов каждого форума. Обычно эта проблема бывает после чистки от спама или после редактирования базы данных.</p>
</div>
<p class="submitend">
...
<form method="post" action="<?php echo $_SERVER['REQUEST_URI']; ?>">
<div class="inbox">
<p>Исправляет список последних постов каждой темы. Обычно эта проблема бывает после чистки от спама или после редактирования базы данных.</p>
</div>
<p class="submitend">
...
<form method="post" action="<?php echo $_SERVER['REQUEST_URI']; ?>">
<div class="inbox">
<p>Удаляет все сообщения чьи родительские темы былы удалены, или наоборот темы которые не содержат сообщений и все темы, чьи родительские форумы были удалены. Обычно эта проблема бывает после редактирования базы данных.</p>
</div>
<p class="submitend">
