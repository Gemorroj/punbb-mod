...
if ($db->num_rows($result)) {
    while ($cur_topic = $db->fetch_assoc($result)) {
        ...
        // REAL MARK TOPIC AS READ MOD BEGIN
        if (!$pun_user['is_guest'] && $cur_topic['moved_to'] == null && !is_reading( $cur_topic['log_time'], $cur_topic['last_post']) && $cur_topic['last_post'] > $cur_topic['mark_read'] && ($cur_topic['last_post'] > $pun_user['last_visit'] || ($_SERVER['REQUEST_TIME'] - $cur_topic['last_post'] < $pun_user['mark_after']))) {
            // REAL MARK TOPIC AS READ MOD END
