...
if (!$pun_user['is_guest']) {
    ...
    if (null == $cur_topic['log_time']) {
        $db->query('INSERT INTO '.$db->prefix.'log_topics (user_id, forum_id, topic_id, log_time) VALUES ('.$pun_user['id'].', '.$cur_topic['forum_id'].', '.$id.', '.$_SERVER['REQUEST_TIME'].')') || \error('Unable to insert reading_mark info', __FILE__, __LINE__, $db->error());
    } else {
...
if (!$cur_topic['closed']) {
    if ((!$cur_topic['post_replies'] && (1 == $pun_user['g_post_replies'] || 2 == $pun_user['g_post_replies'])) || 1 == $cur_topic['post_replies'] || $is_admmod) {
...
foreach ($posts as $cur_post) {
    ...
    if ($cur_post['poster_id'] > 1) {
        ...
        if (1 == $pun_config['o_show_user_info'] && $cur_post['poster_email'] && !$pun_user['is_guest']) {
        ...
        */
        ...
        if (!$cur_topic['closed']) {
            if ($cur_post['poster_id'] == $pun_user['id']) {
                if ((1 == ($start_from + $post_count) && 1 == $pun_user['g_delete_topics']) || (($start_from + $post_count) > 1 && 1 == $pun_user['g_delete_posts'])) {
                    $post_actions[] = '<li class="postdelete"><a href="delete.php?id='.$cur_post['id'].'">'.$lang_topic['Delete'].'</a>';
                }
                if (1 == $pun_user['g_edit_posts']) {
...
$db->query('UPDATE `'.$db->prefix.'topics` SET `num_views`=`num_views`+1 WHERE id='.$id) || \error('Unable to update topic', __FILE__, __LINE__, $db->error());
